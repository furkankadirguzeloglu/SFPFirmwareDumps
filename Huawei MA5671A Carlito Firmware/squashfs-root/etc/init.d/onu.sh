...
ploam_config() {
	...
	if [ -z "$nSerial" ]; then
		...
		nSerial=$(echo $ethaddr | awk 'BEGIN{FS=":"} {print "0x5a 0x52 0x4d 0x54 0x"$3" 0x"$4" 0x"$5" 0x"$6}')
	fi
	onu ploam_init
...
start() {
	...
	# initialize onu driver level PONSTICK custom features
	sple=`uci get sys.features.suppress_power_level` || return 1
	onu onu_suppress_power_level_enable_set ${sple}
	...
	if [ -f "/etc/config/.fttdp" ]; then
		...
		# - uint32_t tmu_global_segments_yellow
		# - uint32_t tmu_global_segments_red
		onu gpe_shared_buffer_cfg_set 576 3000 3000 3000 3000
	fi
}
