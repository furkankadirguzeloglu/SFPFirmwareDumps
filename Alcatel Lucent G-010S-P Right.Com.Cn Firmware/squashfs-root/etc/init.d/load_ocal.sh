...
optic() {
	...
	#echo "result $result"
	status=${result%% *}
	key=`echo $status | cut -d '=' -f 1`
	val=`echo $status | cut -d '=' -f 2`
...
start() {
	...
	bertEnable=`fw_printenv bertEnable 2>&- | cut -f 2 -d '='`
	...
	is1PPS_mode=`fw_printenv is1PPS_mode 2>&- | cut -f 2 -d '='`
	if [ -n "$is1PPS_mode" ]; then
		if [ "$is1PPS_mode" == "true" ]; then
			onu gpetpcs 2 0 0 0 0
		fi
	fi
	asc0=`fw_printenv asc0 2>&- | cut -f 2 -d '='`
	isTod_init=`fw_printenv isTod_init 2>&- | cut -f 2 -d '='`
