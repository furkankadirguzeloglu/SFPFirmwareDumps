...
boot() {
  ...
  sfp_i2c -i 507 -l 1 -p 1
	...
	# set current EEPROM
	sfp_i2c -e $eeprom
	cins=$(cat /usr/etc/buildinfo | grep IMAG |awk -F "=" '{print $2}'| cut -b 1-8)
	if [ $cins == '3FE46398' ];then
	sfp_i2c -i 0x1fc -w 0x02
	else
	sfp_i2c -i 0x1fc -w 0x00
	fi
	echo $cins
...
start() {
	...
	/sbin/sfp_ddm_tool monitor > /dev/console &
	value=$(/sbin/sfp_ddm_tool dumpA0 | grep transceiver_2 | awk -F ':' '{print $2}'|cut -b 2-3)
	if [ $value == 20 ]; then
	/sbin/sfp_ddm_tool writeA0 transceiver_2 0x00
	echo "write transceiver_2 0x00"
	fi
}
