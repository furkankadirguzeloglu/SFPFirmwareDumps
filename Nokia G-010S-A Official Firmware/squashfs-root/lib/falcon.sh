...
falcon_ploam_emergency_stop_state_get() {
	local retval="0"
	local state=`fw_printenv ploam_emergency_stop_state 2>&- | cut -f2 -d=`
...
falcon_goi_calibrated_get() {
	local tmp
	local goi_calibrated="0"

	local tmp=`fw_printenv goi_calibrated 2>&- | cut -f2 -d=`
	if [ -n "$tmp" ]; then
		case "$tmp" in
		1)
			goi_calibrated=$tmp
		...
		config_get tmp "global" "CalibrationTimestamp" 0
		if [ "$tmp" != "0" ]; then
			goi_calibrated="1"
		fi
	fi
