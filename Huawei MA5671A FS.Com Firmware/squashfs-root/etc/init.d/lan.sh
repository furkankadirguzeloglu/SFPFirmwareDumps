...
get_gpon_lan_ports() {
	...
	GFAST*)
		...
		loop_ports="3"
		;;
	easy88388|FTTDP8) # DPU FTTDP8 module
		gmii_port=1
		sgmii_port=0
	...
	# special handling for hgu, to allow loops on specified ports
	[ -f "/etc/config/.hgu" ] && gpon_lan_fixed_rgmii 2 3
}
...
enable_gpon_lan_ports() {
	...
	[ -f "/etc/config/.hgu" ] && {
		# on HGU, the lan ports are not managed by OMCI and must be enabled here
		onu lanpe 0
		onu lanpe 1
	}
}
...
BOARDS="easy98000 easy98000_4fe easy98000_serdes easy98021 easy98020 easy98010 MDU SFP FTTDP8"
